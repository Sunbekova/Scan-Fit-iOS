...
actor AINetworkService {
    ...
    func analyzeIngredientsFull(
        ...
        var body: [String: Any] = ["ingredients": ingredients, "health_info": healthInfo]
        if let pj = productJson {
            body["product_json"] = pj
            if let d = pj.data(using: .utf8), let parsed = try? JSONSerialization.jsonObject(with: d) {
                body["product"] = parsed
            }
        }
        if let uj = userProfileJson {
            body["user_profile_json"] = uj
            if let d = uj.data(using: .utf8), let parsed = try? JSONSerialization.jsonObject(with: d) {
                body["user"] = parsed
            }
        }
    ...
    func analyzeIngredients(ingredients: String, healthInfo: String) async throws -> AnalysisResponse {
        try await analyzeIngredientsFull(ingredients: ingredients, healthInfo: healthInfo)
    }

    func analyzeImageScanWithUserContext(imageData: Data, healthInfo: String, userInformationJson: String?) async throws -> AnalysisResponse {
        guard let url = URL(string: "\(aiBaseURL)/analyze-scan") else { throw NetworkError.invalidURL }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        let boundary = UUID().uuidString
